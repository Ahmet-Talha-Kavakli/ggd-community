...
drop policy if exists "ban_evidence: public read" on ban_evidence;
create policy "ban_evidence: public read"
  on ban_evidence for select using (true);
drop policy if exists "ban_evidence: admin write" on ban_evidence;
create policy "ban_evidence: admin write"
  on ban_evidence for insert with check (is_admin(auth.uid()));
drop policy if exists "ban_evidence: admin delete" on ban_evidence;
...
drop policy if exists "warning_evidence: public read" on warning_evidence;
create policy "warning_evidence: public read"
  on warning_evidence for select using (true);
drop policy if exists "warning_evidence: admin write" on warning_evidence;
create policy "warning_evidence: admin write"
  on warning_evidence for insert with check (is_admin(auth.uid()));
drop policy if exists "warning_evidence: admin delete" on warning_evidence;
...
drop policy if exists "red_zone_evidence: public read active" on red_zone_evidence;
...
drop policy if exists "red_zone_evidence: admin read all" on red_zone_evidence;
create policy "red_zone_evidence: admin read all"
  on red_zone_evidence for select using (is_admin(auth.uid()));
drop policy if exists "red_zone_evidence: admin write" on red_zone_evidence;
create policy "red_zone_evidence: admin write"
  on red_zone_evidence for insert with check (is_admin(auth.uid()));
drop policy if exists "red_zone_evidence: admin delete" on red_zone_evidence;
...
drop policy if exists "admin-evidence: admin upload" on storage.objects;
create policy "admin-evidence: admin upload"
  on storage.objects for insert
  with check (bucket_id = 'admin-evidence' and is_admin(auth.uid()));

drop policy if exists "admin-evidence: public read" on storage.objects;
create policy "admin-evidence: public read"
  on storage.objects for select
  using (bucket_id = 'admin-evidence');

drop policy if exists "admin-evidence: admin delete" on storage.objects;
